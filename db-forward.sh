...
_check_sub_command_ok() {
	...
	if test "#${_sub_command}#" = "#up#"; then
		printf 'OK'
	elif test "#${_sub_command}#" = "#down#"; then
		printf 'OK'
	else
		printf 'NOK'
	fi
...
_run_down() {
	...
	if test "${_port_forward_found}" -gt 0; then
		_port_forward_for_config | awk '{print $2}' | xargs kill
	fi
...
_show_current_port_forward_procs() {
	...
	ps -ef | grep "[k]ubectl .\+ port-forward pod/postgres-forward-pod-"
	printf '.\n'
}
